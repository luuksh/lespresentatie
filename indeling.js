...
function groepjesIndeling() {
  ...
  // Vijf groepjes van vier
  for (let i = 0; i < 5; i++) {
    wrapper.appendChild(maakGroepje(shuffled.slice(index, index + 4)));
    index += 4;
  }

  // Twee groepjes van vijf
  for (let i = 0; i < 2; i++) {
    wrapper.appendChild(maakGroepje(shuffled.slice(index, index + 5)));
    index += 5;
  }
