...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function planningSignalsNoLessons(value) {
  ...
  function weekHasPlannedLessonException(classId, date) {
    const weekInfo = currentWeekCandidates(date);
    const planningEntry = getStudioEntry(classId, weekInfo.keys);
    if (planningEntry) {
      if (planningSignalsNoLessons(planningEntry.note)) return true;
      if ((planningEntry.items || []).some((item) => planningSignalsNoLessons(item))) return true;
    }
    return Boolean(holidayForDate(date));
  }

  function inferClassLessonSlots(entries, classId) {
    const bySlot = new Map();
    const weeklyLessons = new Map();

    for (const entry of entries) {
      if (!classIdMatch(entry.classId, classId)) continue;
      const weekInfo = currentWeekCandidates(entry.start);
      const key = weekInfo?.week?.id || '';
      if (!key) continue;
    ...
    for (const lessonEntries of weeklyLessons.values()) {
      ...
      if (weekHasPlannedLessonException(classId, ordered[0]?.start)) continue;
  ...
  function resolveAgendaFallbackSourceUrl() {
      ...
      if (!images.length && safeMediaUrl && !/^javascript:/i.test(safeMediaUrl)) {
        return `
          <figure class="presentation-slide-media is-video">
            ${externalEmbedHtml(safeMediaUrl, slide.caption || slide.title || 'Open fragment')}
  ...
  function formatAgendaDebug(entries = [], now = new Date()) {
    ...
    const header = [
      ...
      `agenda gebruikt: ${agendaLastResolvedSource || '-'}`,
  ...
  async function fetchAgenda({ source = 'auto' } = {}) {
    ...
    if (agendaSourceUrl) sourceAttempts.push({ url: agendaSourceUrl, label: 'live' });
    if (fallbackSourceUrl && fallbackSourceUrl !== agendaSourceUrl) sourceAttempts.push({ url: fallbackSourceUrl, label: 'fallback' });
    let loaded = false;
    let lastErr = null;
    for (const attempt of sourceAttempts) {
      try {
        const url = new URL(attempt.url, window.location.href);
        url.searchParams.set('_t', String(Date.now()));
        const res = await fetch(url.toString(), { cache: 'no-store' });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        ...
        agendaLastError = '';
        agendaLastResolvedSource = attempt.url;
        loaded = true;
        break;
      } catch (err) {
        lastErr = err;
